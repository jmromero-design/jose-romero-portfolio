...
**35 live pages** as of 2026-09-12 (was 31 as of 2026-06-24; +1 EN/ES pair `/thinking/beco-knowledge-graph/`, a presentation-style visual explainer using its own `bko-` prefixed page-specific styles; +1 EN/ES pair `/thinking/beco-explorer/`, a zoomable treemap-of-cards browser for the same graph, prefixed `bgx-` — see each file itself, neither is a template to generalize). The explorer fetches static JSON from `assets/data/beco-explorer/` (`tree.json` + one file per named bias under `biases/`), regenerated by `scripts/generate_beco_explorer_data.py` — re-run it after any BECO ingest round you want reflected publicly; nothing regenerates automatically. EN pages at root, ES pages under `/es/` with identical structure.
